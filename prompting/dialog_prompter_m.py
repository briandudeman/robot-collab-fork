...
class DialogPrompter:
    ...
    def __init__(
        ...
        robot_name_map: Dict[str, str] = {"panda": "agentA", "panda": "agentB"},
        num_replans: int = 3, 
    ...
    def prompt_one_round(self, obs: EnvState, save_path: str = ""): 
        ...
        for i in range(self.num_replans):
            ...
            parse_succ, parsed_str, llm_plans = self.parser.parse(obs, final_response) # only mention of parser
...
{parsed_str} Re-format to strictly follow [Action Output Instruction]!"""
                ...
                for j, llm_plan in enumerate(llm_plans): 
                    ready_to_execute, env_feedback = self.feedback_manager.give_feedback(llm_plan) # only mention of feedback_manager       
