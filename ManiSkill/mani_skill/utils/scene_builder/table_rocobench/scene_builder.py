...
class RocoTableSceneBuilder(SceneBuilder):
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        b = len(env_idx)
        self.table.set_pose(
            sapien.Pose(p=[0, 0, -0.9196429], q=euler2quat(0, 0, np.pi / 2))
        )
        if self.env.robot_uids == "panda":
            ...
            qpos[:, -2:] = 0.04
            self.env.agent.reset(qpos)
            self.env.agent.robot.set_pose(sapien.Pose([-0.615, 1, 1]))
        ...
        # this is where everything will happen for now
            ...
            #qpos[:, -2:] = 0.04
            #print("joints: ", [j.name for j in agent.agents[1].robot.joints])
            #print("qpos: ", qpos)
            #print(getattr(env.agent.robot, "joint_names", None))
            agent.agents[1].reset(qpos)
