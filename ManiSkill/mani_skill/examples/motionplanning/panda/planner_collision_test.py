...
print("urdf path: ", env.unwrapped.agent.urdf_path)
...
print(env.agent.tcp_pose.p[0])
env.agent.tcp_pose.set_p(torch.add(env.agent.tcp_pose.p[0], .05))
print(env.agent.tcp_pose.p[0])
pose = to_sapien_pose(env.agent.tcp_pose)

print("arg1: ", np.concatenate([torch.add(env.agent.tcp_pose.p[0], .05), pose.q]))
print("arg2: ", env.agent.robot.get_qpos().cpu().numpy()[0])
print("arg3: ", env.control_timestep)
print("arg4: ", False)
result = planner_base.plan_screw(
    np.concatenate([torch.add(env.agent.tcp_pose.p[0], .05), pose.q]),
    env.agent.robot.get_qpos().cpu().numpy()[0],
